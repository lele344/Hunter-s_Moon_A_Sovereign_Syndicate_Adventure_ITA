...
function Get-PackagePaths {
    ...
    return [pscustomobject]@{
        ...
        BackupDll           = Join-Path $PSScriptRoot 'backup\Locale.original.dll'
        BackupJson          = Join-Path $PSScriptRoot 'backup\Italian_translated.original.json'
    }
}
...
function Convert-ToNamespacedTranslations {
    ...
    foreach ($key in $Resources.Keys) {
        ...
        if (-not $Translated.Contains($legacyKey) -or -not $Source.Contains($legacyKey)) {
            continue
        }

        # A legacy flat key belongs to this namespace only when its recorded
        # English source matches. This preserves existing card descriptions
        # without incorrectly copying them into the identically named title.
        if ([string]$Source[$legacyKey] -ceq [string]$Resources[$key]) {
...
function Get-TranslationAnalysis {
    ...
    $migration = Convert-ToNamespacedTranslations -Source $source -Translated $translated -Resources $resources
    ...
    foreach ($key in $resources.Keys) {
        ...
        $legacyKey = Get-LegacyLocaleKey $key
        $hasMatchingLegacySource = $source.Contains($legacyKey) -and ([string]$source[$legacyKey] -ceq $english)
        $hasCurrentSource = $source.Contains($key) -and ([string]$source[$key] -ceq $english)
        if (-not $hasCurrentSource -and -not $hasMatchingLegacySource) {
...
function Build-ReleasePackage {
    ...
    Write-Log "Release aggiornata: Italian_translated.json"
}
...
function Invoke-InstallMod {
    ...
    New-Item -ItemType Directory -Force -Path $Paths.GameManaged, $Paths.GameLocales | Out-Null
    ...
    if ((Test-Path -LiteralPath $Paths.GameLocaleJson) -and -not (Test-Path -LiteralPath $Paths.BackupJson)) {
        ...
        Write-Log 'Backup del JSON originale creato.'
    }
    ...
    Write-Log 'Mod italiana installata nel gioco.'
}

function Invoke-UninstallMod {
    param($Paths)

    New-Item -ItemType Directory -Force -Path $Paths.GameManaged, $Paths.GameLocales, $Paths.BackupRoot | Out-Null
...
if ($Action -eq 'Gui') {
    Ensure-PackageFolders (Get-PackagePaths $GameRoot)
    Show-TranslatorGui
    return
}
